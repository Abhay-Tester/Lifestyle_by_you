...
const formatDateFriendlyLocal = (dateStr: string): string => {
...
export const HabitMatrixSheet: React.FC<HabitMatrixSheetProps> = ({
  ...
  // View Mode: 'matrix' (Full Multi-Day Sheet) or 'cards' (Touch-first Daily Focus Checklist)
  const [viewMode, setViewMode] = useState<'matrix' | 'cards'>(() => {
    return typeof window !== 'undefined' && window.innerWidth < 640 ? 'cards' : 'matrix';
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden space-y-4 p-3 sm:p-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Mark checklist items per date. Switch between touch-friendly Focus View & Matrix Sheet.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-2 sm:gap-3 shrink-0">
            {/* View Mode Toggle: Focus Checklist vs Sheet Table */}
            <div className="flex items-center bg-slate-100 p-1 rounded-xl text-xs font-semibold">
              <button
                ...
                title="Phone Daily Focus Checklist"
              >
        ...
        {viewMode === 'cards' ? (
          /* Phone-first Daily Focus Checklist */
          <div className="space-y-4">
            {/* Selected Date Summary Banner */}
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-900">
                    {formatDateFriendlyLocal(selectedDate)}
                  </span>
                ...
                <div className="flex items-center gap-2 text-xs text-slate-600">
                  <span>
                    {filteredTasks.filter(t => t.completedDates && t.completedDates[selectedDate]).length} of {filteredTasks.length} habits completed
                  </span>
                  <span className="font-bold text-indigo-600">
                    ({filteredTasks.length > 0 ? Math.round((filteredTasks.filter(t => t.completedDates && t.completedDates[selectedDate]).length / filteredTasks.length) * 100) : 0}%)
                  </span>
                </div>
              </div>

              {/* Progress Bar */}
              <div className="w-full sm:w-48 bg-slate-200 h-2.5 rounded-full overflow-hidden shrink-0">
                <div 
                  className="bg-gradient-to-r from-indigo-600 to-emerald-500 h-full rounded-full transition-all duration-300"
                  style={{
                    width: `${filteredTasks.length > 0 ? Math.round((filteredTasks.filter(t => t.completedDates && t.completedDates[selectedDate]).length / filteredTasks.length) * 100) : 0}%`
                  }}
                />
              </div>
            </div>
            ...
            {filteredTasks.length === 0 ? (
              <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-slate-500">
                <p className="text-sm font-medium">No habits listed in this category.</p>
                <button
                  onClick={() => setShowAddModal(true)}
                  className="mt-3 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1.5 cursor-pointer shadow-xs"
                >
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {filteredTasks.map((task, tIdx) => {
                  const isChecked = !!(task.completedDates && task.completedDates[selectedDate]);
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`p-3 sm:p-4 rounded-2xl border transition-all flex items-start gap-3 bg-white ${
                        isChecked 
                          ? 'border-emerald-200 bg-emerald-50/20' 
                          : 'border-slate-200 hover:border-slate-300 shadow-xs'
                      }`}
                    >
                      {/* Touch-Friendly Checkbox Button (44px target) */}
                      <button
                        type="button"
                        onClick={() => onToggleTask(task.id, selectedDate)}
                        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl shrink-0 flex items-center justify-center transition-all cursor-pointer ${
                          isChecked 
                            ? 'bg-emerald-600 text-white shadow-xs scale-105' 
                            : 'border-2 border-slate-300 hover:border-indigo-500 bg-slate-50 text-transparent'
                        }`}
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-1.5 mb-1">
                          <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${colorTheme.badge}`}>
                            {colorTheme.name}
                          </span>
                          {task.priority === 'high' && (
                          ...
                          {streak > 0 && (
                            <span className="px-2 py-0.2 rounded-full bg-amber-100 text-amber-800 text-[10px] font-bold flex items-center gap-0.5">
                      ...
                      {/* Card Actions: Move, Edit & Delete */}
                      <div className="flex flex-col items-center gap-1 shrink-0">
                        {onMoveTask && (
                          <div className="flex flex-col bg-slate-100 rounded-md p-0.5">
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); onMoveTask(task.id, 'up'); }}
                              disabled={tIdx === 0}
                              className="p-1 text-slate-500 hover:text-indigo-600 disabled:opacity-20 cursor-pointer"
                              title="Move Up"
                            >
