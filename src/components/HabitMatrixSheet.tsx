...
export const HabitMatrixSheet: React.FC<HabitMatrixSheetProps> = ({
  ...
  const getCategoryColor = (category: TaskCategory, index: number) => {
    const colors = [
      { bg: 'bg-emerald-500', text: 'text-emerald-700', border: 'border-emerald-600' },
      { bg: 'bg-indigo-600', text: 'text-indigo-700', border: 'border-indigo-600' },
      { bg: 'bg-amber-500', text: 'text-amber-700', border: 'border-amber-600' },
      { bg: 'bg-cyan-500', text: 'text-cyan-700', border: 'border-cyan-600' },
      { bg: 'bg-rose-500', text: 'text-rose-700', border: 'border-rose-600' },
      { bg: 'bg-purple-600', text: 'text-purple-700', border: 'border-purple-600' },
    ];
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            <h2 className="font-bold text-slate-900 text-lg flex items-center gap-2">
              ...
              <span>Habit Matrix Sheet</span>
              <span className="text-xs font-semibold bg-indigo-50 text-indigo-700 px-2.5 py-0.5 rounded-full">
                {filteredTasks.length} Habits
              </span>
            </h2>
            <p className="text-xs text-slate-500 mt-1">
              Mark checklist items per date. Default view displays 3 days (Today, Tomorrow, Day After).
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl text-xs font-medium">
              ...
              <select
                ...
                className="bg-transparent text-slate-700 font-semibold focus:outline-none pr-2 capitalize cursor-pointer"
              >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs shadow-xs transition-all shrink-0"
            >
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            ...
            <thead>
              <tr className="bg-slate-900 text-slate-200 text-xs font-bold uppercase tracking-wider">
                <th 
                  ...
                  style={{ width: habitColWidth, minWidth: habitColWidth, maxWidth: habitColWidth }}
                >
                  <div className="flex items-center justify-between pr-2">
                    <span className="truncate">HABIT LIST ({filteredTasks.length})</span>
                  </div>
                  {/* Mouse Drag Handle to Expand Width */}
                  <div
                    onMouseDown={handleMouseDownResize}
                    className="absolute top-0 right-0 bottom-0 w-2 cursor-col-resize hover:bg-indigo-500/80 active:bg-indigo-500 transition-colors z-20 group"
                    title="Drag left/right to resize column width"
                  >
                    <div className="w-0.5 h-full bg-slate-700 group-hover:bg-indigo-400 mx-auto" />
                  </div>
                </th>
                <th className="p-2 w-16 text-center border-b border-slate-800 border-r border-slate-800">
                  GOAL
                </th>
                ...
                {daysList.map((day) => {
                  ...
                  return (
                    <th 
                      key={day.dateStr}
                      className={`p-1.5 text-center border-b border-r border-slate-800 min-w-[42px] ${
                        day.isToday ? 'bg-indigo-950 text-indigo-200 font-extrabold' : ''
                      }`}
                    >
                      <div className="flex flex-col items-center py-1">
                        {/* Number of tasks completed on this date */}
                        <span className={`text-[11px] font-mono font-bold leading-tight px-1.5 py-0.5 rounded ${
                ...
                <th className="p-3 w-32 text-center border-b border-slate-800 sticky right-0 bg-slate-900 z-10">
                  PROGRESS
                </th>
              </tr>
            </thead>
              ...
              {filteredTasks.length === 0 ? (
                ...
                filteredTasks.map((task, tIdx) => {
                  ...
                  const taskCompletedDaysCount = daysList.filter(
                    (d) => task.completedDates[d.dateStr]
                  ).length;
                  ...
                  return (
                    ...
                    >
                      {/* Habit Name Column */}
                      <td 
                        className="p-2 font-semibold text-slate-900 sticky left-0 bg-white group-hover:bg-slate-50 z-10 border-r border-slate-200 shadow-xs"
                        style={{ width: habitColWidth, minWidth: habitColWidth, maxWidth: habitColWidth }}
                      >
                        <div className="flex items-center justify-between gap-1 group/row">
                          <div className="flex items-center gap-1.5 min-w-0 flex-1">
                            {/* Drag Handle */}
                            <span 
                              className="cursor-grab active:cursor-grabbing p-0.5 text-slate-300 hover:text-slate-600 rounded transition-colors shrink-0"
                              title="Click and drag to reorder habit"
                            >
                              <GripVertical className="w-3.5 h-3.5" />
                            </span>

                            <span className={`w-2.5 h-2.5 rounded-full ${colorTheme.bg} shrink-0`} />
                            
                            <span className="truncate text-slate-900 font-semibold" title={task.title}>
                              {task.title}
                            </span>
                          </div>

                          {/* Quick Actions: Move Up, Move Down, Edit & Delete */}
                          <div className="flex items-center gap-0.5 opacity-70 group-hover:opacity-100 transition-opacity shrink-0">
                            ...
                            {onMoveTask && (
                              <div className="flex items-center bg-slate-100/80 rounded p-0.5 mr-0.5">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  disabled={tIdx === 0}
                                  className="p-0.5 text-slate-500 hover:text-indigo-600 disabled:opacity-30 disabled:hover:text-slate-500 transition-colors"
                                  title="Move Up"
                                >
                                ...
                                <button
                                  ...
                                  disabled={tIdx === filteredTasks.length - 1}
                                  className="p-0.5 text-slate-500 hover:text-indigo-600 disabled:opacity-30 disabled:hover:text-slate-500 transition-colors"
                                  title="Move Down"
                                >
                      ...
                      {daysList.map((day) => {
                        const isChecked = !!task.completedDates[day.dateStr];
                        ...
                        return (
                          ...
                          >
                            <button
                              ...
                              className={`w-7 h-7 rounded-md flex items-center justify-center font-bold font-mono transition-all duration-150 mx-auto text-xs ${
                                isChecked
                      ...
                      {/* Progress Column */}
                      <td className="p-2 sticky right-0 bg-white group-hover:bg-slate-50 z-10 border-l border-slate-200 shadow-xs">
                        <div className="flex items-center gap-2">
                          <span className="font-mono font-bold text-xs text-slate-700 w-10 text-right">
                            {rowPercentage}%
                          </span>
                          <div className="flex-1 bg-slate-100 h-2 rounded-full overflow-hidden border border-slate-200 min-w-[40px]">
                            <div 
            ...
            <tfoot>
              <tr className="bg-slate-900 text-white font-bold text-xs border-t-2 border-slate-800">
                <td 
                  className="p-3 sticky left-0 bg-slate-900 z-10 border-r border-slate-800"
                  style={{ width: habitColWidth, minWidth: habitColWidth, maxWidth: habitColWidth }}
                >
                  <span className="truncate block">TOTAL COMPLETED PER DAY</span>
                </td>
                <td className="p-2 text-center border-r border-slate-800 font-mono text-slate-400">
                  -
                </td>
                ...
                {daysList.map((day) => {
                  ...
                  return (
                    <td 
                      key={day.dateStr}
                      className="p-2 text-center border-r border-slate-800 font-mono font-extrabold text-emerald-400 text-xs"
                    >
                      {dayTotal}
                    </td>
                  );
                ...
                <td className="p-3 sticky right-0 bg-slate-900 z-10 text-center font-mono text-xs text-emerald-400">
